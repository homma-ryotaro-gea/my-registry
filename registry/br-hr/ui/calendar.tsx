...
import { Button, buttonVariants } from "@/registry/br-hr/ui/button";
...
function Calendar({
	...
	return (
		<DayPicker
			...
			formatters={{
				formatCaption: (date) => dayjs(date).format("YYYY年MM月"),
				// ドロップダウン 月
			...
			classNames={{
				...
				dropdowns: cn(
					"flex h-(--cell-size) w-full flex-row-reverse items-center justify-center gap-1.5 font-medium text-sm",
	...
	return (
		<DayPicker
			...
			classNames={{
				...
				month: cn("flex w-full flex-col gap-4", defaultClassNames.month),
				nav: cn(
					"absolute inset-x-0 flex w-full items-center justify-between gap-1",
					captionLayout === "label" ? "top-[-5px]" : "top-0",
					defaultClassNames.nav,
				),
				button_previous: cn(
					buttonVariants({ variant: buttonVariant }),
					"h-[--cell-size] w-[--cell-size] select-none p-2 aria-disabled:opacity-50",
					defaultClassNames.button_previous,
				),
				button_next: cn(
					buttonVariants({ variant: buttonVariant }),
				...
				dropdown: cn(
					"absolute inset-0 bg-popover opacity-0 hover:cursor-pointer",
					defaultClassNames.dropdown,
				),
				caption_label: cn(
					"select-none font-medium",
...
export { Calendar, CalendarDayButton, CustomCalendar };
